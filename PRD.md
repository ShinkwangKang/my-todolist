...
### 1. 뷰 모드
- 상단 탭 또는 토글로 **칸반 보드** / **주간 보드** / **주간 리포트** 뷰 전환
- 동일한 데이터(카드)를 세 가지 관점으로 표시
...
- **캐리오버**: 미완료(Done/Cancel 아닌) 카드는 시작일부터 **min(오늘, 마감일)** 까지 이어서 표시
  - 예: 월요일 시작, 마감일 없음, 오늘 수요일 → 월, 화, 수 컬럼에 표시
  - 예: 월요일 시작, 마감일 화요일, 오늘 수요일 → 월, 화 컬럼에만 표시
  - 미래 요일에는 표시하지 않음 (해당 요일이 되면 자동으로 나타남)
  - 완료/취소된 카드는 시작일부터 **min(마감일, 완료일)** 까지 표시
...
#### 주간 리포트
- 주간 네비게이션 (이전 주 / 이번 주 / 다음 주)
- 한 주(월~일) 단위의 업무 현황을 요약하는 읽기 전용 리포트 뷰
- **섹션 구성**:

| 섹션 | 설명 |
|------|------|
| 이번 주 신규 업무 | 이번 주에 새로 생성된 카드 목록 |
| 지난주 이월 업무 | 지난주 이전에 시작되어 이번 주까지 이어진 미완료 카드 |
| 완료된 업무 | 이번 주에 완료(Done)된 카드 목록 + 완료일 표시 |
| 진행 중 업무 | 주말 기준 아직 미완료인 카드 목록 |
| 취소된 업무 | 이번 주에 Cancel로 이동된 카드 목록 |

- **업무 성격별 통계**:
  - 카테고리별(업무/개인) 카드 수 비율
  - 업무 유형별(개발/문서/JIRA/회의/기타) 카드 수 비율
  - 우선순위별(높음/보통/낮음) 분포
- **요약 지표**:
  - 총 카드 수, 완료율 (완료/전체), 신규 대비 완료 비율
  - 평균 소요 기간 (시작일 → 완료일)
...
- Frontend - 주간 리포트: 섹션별 카드 목록, 업무 성격별 통계, 요약 지표, 클립보드 복사 확인
